def find_next_empty(puzzle):
    # finds the row, col on the puzzle that's not filled yet --> rep with -1
...
def is_valid(puzzle, guess, row, col):
    # figures out whether the guess ar the ow/col of the puzzle is a valid guess
...
def solve_sudoku(puzzle):
    ...
    # return wherter a solution exists
    ...
    for guess in range(1, 10): # range(1, 10) is 1, 2, ..., 9
        ...
        if is_valid(puzzle, guess, row, col):
            # step 3.1: if this valid, then place that guess om the puzzle!
            puzzle[row][col] = guess
            # now recursive using this puzzle!
            #step 4: recursively call our function
            if solve_sudoku(puzzle):
                return True
        # step 5: if not valit OR if our guess does not solve the puzzle, then
        # backtrack and try a new number
        puzzle[row][col] = -1 # reset the guess
        
    # step 6: if none of the numbers    that we that we try work, then this puzzle is UNSOLVABLE!
    return False
